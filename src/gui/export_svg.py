...
class TracePlotter:
    ...
    def _determine_title_font_size(self, samples, row, cols):
        ''' compute the title font size based on the number of sample and length of title '''
        
        name_lengths = [len(x) for x in samples]
        max_name_length = max(name_lengths)

        font_size = 40 / (row + 0.1*max_name_length)
    ...
    def get_smallest_grid(self, n):
        ''' design the grid shape | 2x2 , 3x3 4x4, 5x5, 6x6'''
